...
(function(){
  ...
  document.querySelectorAll('[data-copy]').forEach(b=>b.addEventListener('click',()=>{
    ...
    navigator.clipboard.writeText(text).then(()=>toast('Copied!')).catch(()=>{
      el.select && el.select(); document.execCommand('copy'); toast('Copied!');
    });
  }));
  ...
  if($('pwGen')){
    ...
    const score=(p)=>{let s=0;if(!p)return 0;if(p.length>=8)s++;if(p.length>=12)s++;if(p.length>=16)s++;if(/[A-Z]/.test(p))s++;if(/[a-z]/.test(p))s++;if(/\d/.test(p))s++;if(/[^A-Za-z0-9]/.test(p))s++;return Math.min(s,6);};
    const render=(p)=>{out.value=p;const s=score(p);bar.style.width=(s/6*100)+'%';bar.style.background=s<=2?'#ef4444':s<=4?'#f59e0b':'#22c55e';lab.textContent='Strength: '+(s===0?'—':s<=2?'Weak':s<=4?'Medium':'Strong');};
    const gen=()=>{let pool='';if($('pwUp').checked)pool+=sets.up;if($('pwLow').checked)pool+=sets.low;if($('pwNum').checked)pool+=sets.num;if($('pwSym').checked)pool+=sets.sym;if(!pool){toast('Pick at least one set');return;}const n=+lenR.value;const arr=new Uint32Array(n);crypto.getRandomValues(arr);let p='';for(let i=0;i<n;i++)p+=pool[arr[i]%pool.length];render(p);};
    lenR.addEventListener('input',()=>{lenV.textContent=lenR.value;});
    $('pwGen').addEventListener('click',gen);
    const chk=$('pwCheck'); if(chk) chk.addEventListener('input',e=>render(e.target.value));
    gen();
  }

  /* QR Code */
  if($('qrGen')){
    const upd=()=>{const t=encodeURIComponent($('qrInput').value||'https://example.com');const s=$('qrSize').value;const url='https://api.qrserver.com/v1/create-qr-code/?size='+s+'x'+s+'&data='+t;$('qrImg').src=url;$('qrDownload').href=url;};
    $('qrSize').addEventListener('input',()=>{$('qrSizeVal').textContent=$('qrSize').value;upd();});
    $('qrInput').addEventListener('input',upd);
  ...
  if($('imgFile')){
    ...
    let src=null, origSize=0;
    q.addEventListener('input',()=>{qv.textContent=q.value;if(src)process();});
    fmt.addEventListener('change',()=>{if(src)process();});
    f.addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;origSize=file.size;const r=new FileReader();r.onload=ev=>{const img=new Image();img.onload=()=>{src=img;process();};img.src=ev.target.result;};r.readAsDataURL(file);});
    function process(){c.width=src.width;c.height=src.height;c.getContext('2d').drawImage(src,0,0);c.toBlob(b=>{const url=URL.createObjectURL(b);dl.href=url;dl.download='compressed.'+(fmt.value.split('/')[1]);dl.classList.remove('disabled');info.innerHTML='Original: <b>'+(origSize/1024).toFixed(1)+' KB</b> &middot; '+src.width+'×'+src.height+'<br>Compressed: <b>'+(b.size/1024).toFixed(1)+' KB</b> ('+Math.round((1-b.size/origSize)*100)+'% smaller)';},fmt.value,+q.value);}
  }
  ...
  if($('colorGen')){
    ...
    const hslToHex=(h,s,l)=>{s/=100;l/=100;const a=s*Math.min(l,1-l);const f=n=>{const k=(n+h/30)%12;const c=l-a*Math.max(-1,Math.min(k-3,Math.min(9-k,1)));return Math.round(255*c).toString(16).padStart(2,'0');};return'#'+f(0)+f(8)+f(4);};
    ...
    $('colorGen').addEventListener('click',gen);
    $('colorBase').addEventListener('input',gen);
    $('colorMode').addEventListener('change',gen);
    gen();
  }

  /* Unit Converter */
  if($('unitCat')){
    const data={length:{m:1,km:1000,cm:.01,mm:.001,mi:1609.34,yd:.9144,ft:.3048,in:.0254},weight:{kg:1,g:.001,lb:.453592,oz:.0283495,t:1000},time:{s:1,min:60,h:3600,day:86400,wk:604800}};
    const fill=()=>{const cat=$('unitCat').value;const opts=cat==='temperature'?['C','F','K']:Object.keys(data[cat]);$('unitFrom').innerHTML='';$('unitTo').innerHTML='';opts.forEach(o=>{$('unitFrom').appendChild(new Option(o,o));$('unitTo').appendChild(new Option(o,o));});$('unitTo').selectedIndex=Math.min(1,opts.length-1);calc();};
    ...
    $('unitCat').addEventListener('change',fill);
    ['input','change'].forEach(ev=>{$('unitVal').addEventListener(ev,calc);$('unitFrom').addEventListener(ev,calc);$('unitTo').addEventListener(ev,calc);});
    fill();
  }
  ...
  if($('b64Enc')){
    ...
    $('b64Dec').addEventListener('click',()=>{try{$('b64Out').value=decodeURIComponent(escape(atob($('b64In').value.trim())));}catch(e){$('b64Out').value='Error: invalid Base64';}});
  }
  ...
  if($('urlAnalyze')){
    ...
    $('urlAnalyze').addEventListener('click',run);
  }
})();
